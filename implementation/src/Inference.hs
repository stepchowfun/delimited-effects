...
check c em (EAbs x e) (TArrow t2 r2 t1 r1) r3 = do
  ...
  let h2 = if subrow (eff h1) r3 then HEmpty else h1
  return h2
...
check c em (ETAbs a1 e) (TForall a2 t r1) r2 = do
  ...
  assert (Maybe.isJust $ contextLookupKind c a1)
  ...
  let h2 = if subrow (eff h1) r2 then HEmpty else h1
  return h2
...
infer c _ (EVar x) r1 = do
  ...
  let h = if subrow r2 r1 then HEmpty else HSingleton (EVar x) t r2
  return (t, h)
...
infer c em (EApp e1 e2) r3 = do
  ...
  case t3 of
    TArrow t2 r2 t1 r1 -> do
      ...
      let
        h3 = case (subrow r1 r3, subrow (eff h2) r3) of
          (True , True ) -> h1
