...
check :: Context
      ...
      -> Type
      -> Row
      -> Partial (Row, VarSet String)
check c em (EAbs x e) (TArrow t2 r2 t1 r1) r3 =
  do (r4, v1) <- check (CTExtend c x t2 r2) em e t1 r1
...
infer :: Context
      ...
      -> Term
      -> Row
      -> Partial (Type, Row, VarSet String)
infer c _ (EVar x) r1 =
...
infer _ _ (ETAbs _ _) _ = undefined
infer _ _ (ETApp _ _) _ = undefined
